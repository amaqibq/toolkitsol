...
export default function ImageConverterPage() {
  ...
  const convertImage = async (imageFile: ImageFile): Promise<Blob> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        // Preserve original resolution (no upscaling), but render with high-quality sampling
        canvas.width = img.width
        canvas.height = img.height

        ctx.imageSmoothingEnabled = true
        // Use high-quality resampling where supported – helps keep detail at lower quality settings
        // while still allowing the encoder to shrink file size.
        ...
        const mimeType = `image/${conversionOptions.format}`
        const quality =
          conversionOptions.format === "jpeg" || conversionOptions.format === "webp"
            ? // Clamp between 0.5–0.95 to avoid huge files but keep visual quality
              Math.min(0.95, Math.max(0.5, conversionOptions.quality / 100))
            : undefined

        canvas.toBlob(
          (blob) => {
            // Fallback to original file if encoding somehow fails
            resolve(blob ?? imageFile.file)
          },
          mimeType,
          quality,
        )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(buildToolJsonLd("image-converter")) }}
      />
              ...
              <div className="bg-white/70 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
                ...
                >
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div>
                      <p className="text-lg font-medium text-slate-700 mb-2">Drop your images here or click to browse</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div className="space-y-3">
                    ...
                    <div className="p-3 rounded-xl bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200">
                      <p className="text-sm font-bold text-blue-800">Selected: {conversionOptions.format.toUpperCase()}</p>
